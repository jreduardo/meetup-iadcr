...
modelList <- list("Poisson" = m3P, "Quasi" = m3Q, "BinNeg" = m3B)
...
## Análise de diagnóstico
boot::glm.diag.plots(m3Q)
boot::glm.diag.plots(m3B)
...
aux <- exp(confint(glht(m3B, linfct = X),
                   calpha = univariate_calpha())$confint)
## colnames(aux) <- c("Qfit", "Qlwr", "Qupr")
pred <- rbind(pred, aux)
